# app/models/ead_digital_object.rb
# a Fedora object for an EAD digital objecttype

module DRI
  module Model
  class EADComponent < DigitalObject
    ...
    @@whitelist = { :pdf => {
                    ...
                    :audio => {
                      :type => "audio",
                      :subtypes => ["mp3","mpeg","mpeg3","mp2"],
                      :mime_type => "audio/*"
                    }
                  }

    # Set relationships rules
    belongs_to :governing_collection, :property=>:is_governed_by, :class_name => 'ActiveFedora::Base'
    has_many :governed_items, :property=>:is_governed_by, :inbound => :true, :class_name => 'DRI::Model::EADComponent'
    ...
    has_metadata :name => "descMetadata", :type=> DRI::Metadata::EncodedArchivalDescriptionComponent
    ...
    delegate :ead_level, :to=>"descMetadata", :unique=>"true"
    ...
    def to_solr(solr_doc=Hash.new)
      ...
      # The object type is based on the file type, unless 
      object_type = ead_level
      if (ead_level.downcase == 'file')
      #solr_doc.merge!(solr_name('object_type', :stored_searchable) => "Audio")
      #solr_doc.merge!(solr_name('object_type', :facetable) => "Audio")
      else
        ead_level = ead_level.capitalize
        solr_doc.merge!(solr_name('object_type', :stored_searchable) => ead_level)
        solr_doc.merge!(solr_name('object_type', :facetable) => ead_level)
      end
    ...
    def load_from_xml(xml)
      DRI::Metadata::EncodedArchivalDescriptionComponent.from_xml(xml)
    end
    ...
    def whitelist_type
      ...
      return @@whitelist[properties.object_type][:type]
    end
    ...
    def whitelist_subtypes
      ...
      return @@whitelist[properties.object_type][:subtypes]
    end
    ...
    def mime_type
      ...
      return @@whitelist[properties.object_type][:mime_type]
    end
