  ...
  class QualifiedDublinCore < DRI::Batch
    ...
    def process_collection_relationships
      if self.is_root_collection?
        ...
        Sufia.queue.push(CreateQdcRelationshipsJob.new(self.id))
    ...
    def add_dm_relationship(rels_array, rels_name)
      ...
      if (solr_docs == nil || solr_docs == [])
        logger.error("Solr document for object with PID #{self.id} not found in Solr")
        return false
      end
      ...
      if (root_collection == nil)
        logger.error("Root collection ID for object with PID #{self.id} not found in Solr")
        return false
      end
