    ...
    class EncodedArchivalDescription < DRI::Metadata::Base
      ...
      def to_solr(solr_doc=Hash.new, opts = {})
        ...
        pdate_array = published_date.collect.with_index do |value, idx|
         unless published_date(idx).normal_at.empty?
      ...
      def creator_for_index
        creators_array = []
        ng_xml.search('/ead/archdesc/did/origination/*[not(@role="contributor")]').each { |n| creators_array << (n['role'].nil? ? n.content : "#{n.content} (#{n['role']})") }
      ...
      def add_desc_scope_content(desc_array)
        ng_xml.search('/ead/archdesc/scopecontent').each do |n|
          n.remove
        end
      ...
      def add_desc_abstract(desc_array)
        ng_xml.search('/ead/archdesc/did/abstract').each do |n|
          n.remove
        end
      ...
      def add_desc_biog_hist(desc_array)
        ng_xml.search('/ead/archdesc/bioghist').each do |n|
          n.remove
        end
      ...
      def add_creation_date(dates)
        ...
        if dates.is_a?(Hash) && [:display, :normal].all? { |s| dates.key? s } && dates[:display].size == dates[:normal].size
          ...
          ng_xml.search('/ead/archdesc/did/unitdate[@datechar[contains(translate(., "ABCDEFGHJIKLMNOPQRSTUVWXYZ", "abcdefghjiklmnopqrstuvwxyz"), "creation")]]').each do |n|
            n.remove
          end
      ...
      def add_published_date(dates)
        dates.symbolize_keys! if dates.is_a?(Hash)
        if dates.is_a?(Hash) && [:display, :normal].all? { |s| dates.key? s } &&
            (dates[:display].size == dates[:normal].size)

          ng_xml.search('/ead/eadheader/filedesc/publicationstmt/date').each do |n|
            n.remove
          end
      ...
      def add_contributor(contributors)
        ng_xml.search('/ead/archdesc/did/origination/persname[@role="contributor"]').each do |n|
          n.remove
        end
      ...
      def add_temporal_coverage(dates)
        ...
        if dates.is_a?(Hash) && [:display, :normal, :datechar].all? { |s| dates.key? s } &&
          ...
          ng_xml.search('/ead/archdesc/did/unitdate[not(contains(@datechar, "creation"))]').each do |n|
            n.remove
          end
      ...
      def add_related_material(materials)
        ...
        ng_xml.search('/ead/archdesc/relatedmaterial').each do |n|
          n.remove
        end
      ...
      def add_alternative_form(materials)
        ...
        ng_xml.search('/ead/archdesc/altformavail').each do |n|
          n.remove
        end
      ...
      def add_geogname_coverage_access(locations)
        ...
        if locations.is_a?(Hash) && [:type, :display].all? { |s| locations.key? s } &&
          ...
          ng_xml.search('/ead/archdesc/controlaccess/geogname[not(@role="subject")]').each do |n|
            n.remove
          end
      ...
      def add_language(languages)
        ...
        if languages.is_a?(Hash) && [:langcode, :text].all? { |s| languages.key? s } &&
          ...
          ng_xml.search('/ead/archdesc/did/langmaterial/language').each do |n|
            n.remove
          end
      ...
      def custom_validations
        ...
        # Creation date
        creation_date.each do |curr_cdate|
          creation_date_result = true unless curr_cdate.blank?
        end
        ...
        # EAD-specific
        unless !DRI::Vocabulary.ead_level_values.include?(ead_level.first)
          ead_level.each do |curr_ead_level|
            ead_level_result = true unless curr_ead_level.blank?
          end

          ead_level_other.each do |curr_ead_level|
            ead_level_other_result = true unless curr_ead_level.blank?
          end
        end
